...
local function hasKey(identifier, keyType, keyRef, src)
    ...
    local rows = MySQL.query.await(
        'SELECT * FROM qbx_rp_core_keys WHERE holder = ? AND key_type = ? AND key_ref = ?',
...
exports('GiveKey', function(holderIdentifier, keyType, keyRef, ownerIdentifier, label, temporary, minutes)
    ...
    return MySQL.insert.await(
        'INSERT INTO qbx_rp_core_keys (owner, holder, key_type, key_ref, label, temporary, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        {
...
function Core.EnsureVehicleKey(src, plate, label, notifyKey)
    ...
    local exists = MySQL.single.await(
        'SELECT id FROM qbx_rp_core_keys WHERE holder = ? AND key_type = ? AND key_ref = ? AND temporary = 0 LIMIT 1',
        { id, 'vehicle', plate }
    )
    ...
    elseif Config.Keys.notifyOnGive and created then
        TriggerClientEvent('qbx_rp_core:notify', src, Core.Locale('key_received', plate), 'success')
    end
...
AddEventHandler('qbx_rp_core:keys:giveVehicleKey', function(src, plate, label)
...
CreateThread(function()
    ...
    Core.RegisterUsableItem(item, function(playerId)
        TriggerClientEvent('qbx_rp_core:keys:useItem', playerId)
    end)
end)

lib.callback.register('qbx_rp_core:keys:canLock', function(source, plate)
    local id = Core.GetIdentifier(source)
    if not id then return false end
    return hasKey(id, 'vehicle', plate, source)
end)

lib.callback.register('qbx_rp_core:keys:canHouse', function(source, houseId)
    local id = Core.GetIdentifier(source)
    if not id then return false end
    return hasKey(id, 'house', houseId, source)
end)

lib.callback.register('qbx_rp_core:keys:list', function(source)
    local id = Core.GetIdentifier(source)
    if not id then return {} end

    local rows = MySQL.query.await(
        'SELECT id, key_type, key_ref, label, temporary, expires_at, owner FROM qbx_rp_core_keys WHERE holder = ?',
        { id }
    ) or {}
...
RegisterNetEvent('qbx_rp_core:keys:give', function(targetId, keyType, keyRef, temporary, minutes)
    local src = source
    ...
    if not ok then
        TriggerClientEvent('qbx_rp_core:notify', src, Core.Locale('key_no_key'), 'error')
        return
    end
    ...
    if keyType == 'vehicle' then
        ...
        local row = MySQL.single.await(
            'SELECT 1 FROM qbx_rp_core_keys WHERE owner = ? AND key_type = ? AND key_ref = ? LIMIT 1',
    ...
    TriggerClientEvent('qbx_rp_core:notify', src, Core.Locale('key_given', keyRef), 'success')
    TriggerClientEvent('qbx_rp_core:notify', target.source, Core.Locale('key_received', keyRef), 'inform')
...
RegisterNetEvent('qbx_rp_core:keys:remove', function(keyId)
    local src = source
    local xPlayer = Core.GetPlayer(src)
    ...
    local row = MySQL.single.await('SELECT * FROM qbx_rp_core_keys WHERE id = ? LIMIT 1', { keyId })
    ...
    MySQL.update.await('DELETE FROM qbx_rp_core_keys WHERE id = ?', { keyId })
    TriggerClientEvent('qbx_rp_core:notify', src, Core.Locale('key_removed'), 'success')
...
RegisterNetEvent('qbx_rp_core:keys:setLock', function(netId, plate, locking)
    local src = source
    local id = Core.GetIdentifier(src)
    if not id then return end
    plate = Core.NormalizePlate(plate)
    ...
    TriggerClientEvent('qbx_rp_core:keys:applyLock', -1, netId, locking and true or false, plate, src)
end)

RegisterNetEvent('qbx_rp_core:keys:toggleHouse', function(houseId, locked)
    local src = source
    local id = Core.GetIdentifier(src)
    if not id then return end
    houseId = tostring(houseId or '')
    if not hasKey(id, 'house', houseId, src) then
        TriggerClientEvent('qbx_rp_core:notify', src, Core.Locale('key_house_no_key'), 'error')
        return
    end
    TriggerClientEvent('qbx_rp_core:keys:setHouseLock', -1, houseId, locked and true or false)
    TriggerClientEvent(
        'qbx_rp_core:notify',
        src,
...
exports('RegisterHouseDoor', function(houseId, coords, locked)
    TriggerClientEvent('qbx_rp_core:keys:registerHouse', -1, houseId, coords, locked ~= false)
end)
