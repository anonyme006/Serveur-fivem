...
local function log(msg)
    if Config.AutoImpound.log then
        print(('^3[qbx_rp_core]^0 %s'):format(msg))
    end
end
    ...
    CreateThread(function()
        ...
        for _, row in ipairs(rows) do
            ...
            if player and player.PlayerData and player.PlayerData.source then
                TriggerClientEvent('qbx_rp_core:notify', player.PlayerData.source, Core.Locale('impound_reboot', row.plate), 'inform')
            end
        end
    end)
end
...
lib.callback.register('qbx_rp_core:saveVehicle', function(source, plate, props)
...
RegisterNetEvent('qbx_rp_core:persistDamage', function(plate, props)
    local src = source
