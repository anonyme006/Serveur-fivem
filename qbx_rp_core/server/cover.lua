...
local function loadCovers()
    local rows = MySQL.query.await('SELECT * FROM qbx_rp_core_covers') or {}
    covers = {}
    ...
    TriggerClientEvent('qbx_rp_core:cover:sync', -1, covers)
end
...
lib.callback.register('qbx_rp_core:cover:getAll', function()
    return covers
end)

lib.callback.register('qbx_rp_core:cover:put', function(source, plate, coords, props)
    ...
    MySQL.insert.await(
        'INSERT INTO qbx_rp_core_covers (plate, owner, coords, props) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE owner = VALUES(owner), coords = VALUES(coords), props = VALUES(props)',
...
lib.callback.register('qbx_rp_core:cover:remove', function(source, plate)
    local xPlayer = Core.GetPlayer(source)
    ...
    MySQL.update.await('DELETE FROM qbx_rp_core_covers WHERE plate = ?', { plate })
    covers[plate] = nil
...
AddEventHandler('playerJoining', function()
    ...
    SetTimeout(5000, function()
        TriggerClientEvent('qbx_rp_core:cover:sync', src, covers)
    end)
end)
