...
RegisterNetEvent('qbx_rp_core:cover:sync', function(data)
    syncAll(data)
end)

CreateThread(function()
    Wait(2000)
    local data = lib.callback.await('qbx_rp_core:cover:getAll', false)
...
local function putCover()
    ...
    local ok, msg = lib.callback.await('qbx_rp_core:cover:put', false, plate, {
...
local function removeCover()
    ...
    local ok, msg, entry = lib.callback.await('qbx_rp_core:cover:remove', false, nearestPlate)
    if not ok then
