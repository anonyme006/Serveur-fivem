...
local function toggleLock()
    ...
    local allowed = lib.callback.await('qbx_rp_core:keys:canLock', false, plate)
    if not allowed then
    ...
    TriggerServerEvent('qbx_rp_core:keys:setLock', netId, plate, locking)
end

RegisterNetEvent('qbx_rp_core:keys:applyLock', function(netId, locking, plate, bySrc)
...
RegisterNetEvent('qbx_rp_core:keys:useItem', function()
    toggleLock()
end)
...
RegisterNetEvent('qbx_rp_core:keys:registerHouse', function(houseId, coords, locked)
...
RegisterNetEvent('qbx_rp_core:keys:setHouseLock', function(houseId, locked)
            ...
            if dist < (Config.Keys.houseDistance or 2.5) + 8.0 then
                ...
                if dist < (Config.Keys.houseDistance or 2.5) then
                    ...
                    if IsControlJustReleased(0, 38) then -- E
                        local ok = lib.callback.await('qbx_rp_core:keys:canHouse', false, houseId)
                        if not ok then
                            Core.Notify(Core.Locale('key_house_no_key'), 'error')
                        else
                            TriggerServerEvent('qbx_rp_core:keys:toggleHouse', houseId, not door.locked)
                        end
                    end
                end
            end
