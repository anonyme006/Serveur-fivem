...
local RCORE_CLOTHING = 'rcore_clothing'
...
---@return boolean
local function hasRcoreClothing()
    return GetResourceState(RCORE_CLOTHING) == 'started'
end
...
local function writeSlot(ped, variant, drawable, texture)
    local bridged = hasAppearanceResource() and not hasRcoreClothing()
...
local function loadSavedAppearance(force)
    ...
    if not hasAppearanceResource() and not hasRcoreClothing() then return end
    if savedAppearance and not force then return end

    loading = true

    CreateThread(function()
        if hasRcoreClothing() then
            savedAppearance = nil
        elseif hasAppearanceResource() then
...
AddEventHandler('rcore_clothing:afterSkinLoaded', function() loadSavedAppearance(true) end)
AddEventHandler('rcore_clothing:onClothingShopClosed', function() loadSavedAppearance(true) end)
