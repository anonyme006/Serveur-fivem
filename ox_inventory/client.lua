...
--- Barre capsule orange (esx_progressbar) à la place de ox_lib progress
local function invProgressBar(opts)
...
local function invProgressCircle(opts)
	if GetResourceState('esx_progressbar') == 'started' then
		return exports['esx_progressbar']:progressCircle(opts)
	end
	return lib.progressCircle(opts)
end

local function invProgressActive()
	if GetResourceState('esx_progressbar') == 'started' then
...
lib.callback.register('ox_inventory:usingItem', function(data, noAnim)
	...
	if item and usingItem then
		...
		local success = (not item.usetime or noAnim or invProgressBar({
...
local function useButton(id, slot)
	if PlayerData.loaded and not invBusy and not invProgressActive() then
		local item = PlayerData.inventory[slot]
		if not item then return end
...
local function registerCommands()
	...
	lib.addKeybind({
		...
		onPressed = function()
			if EnableWeaponWheel or not invHotkeys or IsNuiFocused() or invProgressActive() then return end
