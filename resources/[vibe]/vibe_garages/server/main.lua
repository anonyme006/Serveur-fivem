lib.callback.register('vibe_garages:server:list', function(source, garageId)
...
RegisterNetEvent('vibe_garages:server:spawn', function(garageId, plate)
    local src = source
    ...
    if not row then
        ...
        TriggerClientEvent('vibe_garages:client:spawnVehicle', src, 'sultan', garage.spawn, plate or 'DEMO001')
        return
    end
    ...
    TriggerClientEvent('vibe_garages:client:spawnVehicle', src, model, garage.spawn, row.plate)
end)
