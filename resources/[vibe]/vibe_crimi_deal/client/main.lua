                ...
                options[#options+1] = {
                    ...
                    onSelect = function()
                        if lib.progressCircle({
                            ...
                            TriggerServerEvent('vibe_crimi_deal:server:sell', item, { x = c.x, y = c.y, z = c.z })
                        end
                    end,
                }
            end
            lib.registerContext({ id = 'vibe_deal', title = 'Deal', options = options })
            lib.showContext('vibe_deal')
        end,
    },
})
