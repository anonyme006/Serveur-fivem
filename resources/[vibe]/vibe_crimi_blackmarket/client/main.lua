    ...
    for _, it in ipairs(Config.Items) do
        if not it.sell then
            options[#options+1] = {
                ...
                onSelect = function()
                    TriggerServerEvent('vibe_crimi_blackmarket:server:buy', it.item)
                end,
            }
        else
            options[#options+1] = {
                ...
                onSelect = function()
                    TriggerServerEvent('vibe_crimi_blackmarket:server:sell', it.item)
                end,
            }
        end
    end
    lib.registerContext({ id = 'vibe_blackmarket', title = 'Marché noir', options = options })
    lib.showContext('vibe_blackmarket')
end
