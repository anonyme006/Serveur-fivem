...
RegisterCommand('nitro', function()
    if not cache.vehicle or cache.seat ~= -1 then
        exports.vibe_api:Notify('Nitro', 'Conducteur requis.', 'error')
        return
    end
    if boosting then return end
    local ok = lib.callback.await('vibe_crimi_nitro:server:use', false)
    if not ok then
        exports.vibe_api:Notify('Nitro', 'Pas de bouteille nitro.', 'error')
        return
    end
    boosting = true
