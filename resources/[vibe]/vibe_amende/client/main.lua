RegisterCommand('amende', function()
    if not exports.vibe_api:IsPolice(true) then
        exports.vibe_api:Notify('Amende', 'Réservé aux FDO en service.', 'error')
        return
    end
    ...
    TriggerServerEvent('vibe_amende:server:fine', tonumber(input[1]), tonumber(input[2]), input[3])
end, false)
...
exports('OpenFineMenu', function(targetId)
    ...
    if not input then return end
    TriggerServerEvent('vibe_amende:server:fine', targetId, tonumber(input[1]), input[2])
end)
