...
RegisterCommand('course', function()
    ...
    if #(GetEntityCoords(cache.ped) - Config.Start) > 15.0 then
        exports.rr_api:Notify('Course', 'Va au point de depart.', 'error')
        return
    end
    racing = true
    exports.rr_api:Notify('Course', 'GO ! Passe les checkpoints.', 'success')
    CreateThread(function()
        for i, cp in ipairs(Config.Checkpoints) do
            ...
            RemoveBlip(blip)
            exports.rr_api:Notify('Course', ('Checkpoint %s/%s'):format(i, #Config.Checkpoints), 'inform')
        end
        if racing then
            TriggerServerEvent('rr_race:server:finish')
        end
        racing = false
    end)
end, false)
