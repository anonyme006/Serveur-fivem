local function openGarage(garage)
    local vehicles = lib.callback.await('rr_garages:server:list', false, garage.id)
    ...
    for _, veh in ipairs(vehicles) do
        options[#options + 1] = {
            ...
            onSelect = function()
                TriggerServerEvent('rr_garages:server:spawn', garage.id, veh.plate)
            end,
        }
    end

    lib.registerContext({ id = 'rr_garage_' .. garage.id, title = garage.label, options = options })
    lib.showContext('rr_garage_' .. garage.id)
end
...
RegisterNetEvent('rr_garages:client:spawnVehicle', function(model, coords, plate)
