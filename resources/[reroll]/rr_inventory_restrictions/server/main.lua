...
CreateThread(function()
    if not exports.ox_inventory or not exports.ox_inventory.registerHook then
        print('[rr_inventory_restrictions] registerHook indisponible — module inactif')
        return
    end
    ...
    exports.ox_inventory:registerHook('swapItems', function(payload)
        ...
        if restrict then
            ...
            if not job or not restrict[job.name] or not job.onduty then
                exports.rr_api:Notify(src, 'Inventaire', 'Item reserve a un metier en service.', 'error')
                return false
            end
        end
        ...
        if isWeapon and countWeapons(src) >= Config.MaxWeapons then
            exports.rr_api:Notify(src, 'Inventaire', 'Limite d armes atteinte.', 'error')
            return false
        end
        return true
    end, {})
