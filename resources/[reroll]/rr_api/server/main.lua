local function dbg(...)
    ...
    print(('[rr_api] %s'):format(table.concat(parts, ' ')))
end
...
local function hasPoliceJob(src, requireDuty)
    local job = getJob(src)
    if not job or not RerollJobs.IsPolice(job.name) then return false end
...
local function addMoney(src, account, amount, reason)
    ...
    player.Functions.AddMoney(account or 'cash', amount, reason or 'reroll')
    return true
end
...
local function removeMoney(src, account, amount, reason)
    ...
    amount = math.floor(tonumber(amount) or 0)
    if amount <= 0 then return false end
    return player.Functions.RemoveMoney(account or 'cash', amount, reason or 'reroll')
end
...
function SavePlayerMeta(citizenid, stats)
    MySQL.insert.await([[
        INSERT INTO rr_player_meta (citizenid, stats)
        VALUES (?, ?)
...
function LoadPlayerMeta(citizenid)
    local row = MySQL.single.await('SELECT stats, permits FROM rr_player_meta WHERE citizenid = ?', { citizenid })
...
lib.callback.register('rr_api:server:getCitizenId', function(source)
    return getCitizenId(source)
end)

lib.callback.register('rr_api:server:getJob', function(source)
    return getJob(source)
end)

lib.callback.register('rr_api:server:isPolice', function(source, requireDuty)
    return hasPoliceJob(source, requireDuty)
end)

AddEventHandler('onResourceStart', function(res)
    if res ~= GetCurrentResourceName() then return end
    dbg('API reroll démarrée')
end)
