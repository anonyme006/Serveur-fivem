...
RegisterCommand('911', function(_, args)
    ...
    if GetGameTimer() - lastCall < Config.Cooldown * 1000 then
        exports.rr_api:Notify('911', 'Patiente avant un nouvel appel.', 'error')
        return
    end
    lastCall = GetGameTimer()
    local coords = GetEntityCoords(cache.ped)
    TriggerServerEvent('rr_dispatch:server:alert', '911', msg, { x = coords.x, y = coords.y, z = coords.z })
end, false)

RegisterNetEvent('rr_dispatch:client:alert', function(data)
    exports.rr_api:Notify(('Dispatch — %s'):format(data.code), data.message, 'inform')
...
exports('SendAlert', function(code, message, coords)
    ...
    TriggerServerEvent('rr_dispatch:server:alert', code, message, { x = coords.x, y = coords.y, z = coords.z })
end)
