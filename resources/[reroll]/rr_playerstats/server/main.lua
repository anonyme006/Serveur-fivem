...
local function sync(src)
    ...
    if stats then TriggerClientEvent('rr_playerstats:client:sync', src, stats) end
end
...
RegisterNetEvent('rr_playerstats:server:add', function(key, amount)
    local src = source
    local cid, stats = ensure(src)
    ...
    exports.rr_api:SavePlayerMeta(cid, stats)
    sync(src)
end)
    ...
    while true do
        ...
        for _, id in pairs(GetPlayers()) do
            ...
            if stats then
                ...
                exports.rr_api:SavePlayerMeta(cid, stats)
                TriggerClientEvent('rr_playerstats:client:sync', id, stats)
            end
        end
    end
end)
