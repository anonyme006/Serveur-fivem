...
RegisterCommand('ems', function()
    if not isEms() then
        exports.rr_api:Notify('EMS', 'Service EMS requis.', 'error')
        return
    end
    local target = closestPlayer(3.0)
    if not target then
        exports.rr_api:Notify('EMS', 'Aucun patient.', 'error')
        return
    end
    lib.registerContext({
        id = 'rr_ems',
        ...
        options = {
            {
                ...
                onSelect = function()
                    if lib.progressCircle({
                        ...
                        TriggerServerEvent('rr_medicextract:server:revive', target)
                    end
                end,
            },
            {
                ...
                onSelect = function()
                    if lib.progressCircle({
                        ...
                        TriggerServerEvent('rr_medicextract:server:heal', target)
                    end
                end,
            },
            {
                ...
                icon = 'truck-medical',
                onSelect = function()
                    TriggerServerEvent('rr_medicextract:server:extract', target)
                end,
            },
        },
    })
    lib.showContext('rr_ems')
end, false)
...
AddEventHandler('gameEventTriggered', function(name, args)
    ...
    if IsEntityDead(cache.ped) or GetEntityHealth(cache.ped) <= 0 then
        if not dead then
            dead = true
            exports.rr_api:Notify('EMS', 'Tu es inconscient. Attends les secours.', 'error')
        end
    end
end)

RegisterNetEvent('rr_medicextract:client:revive', function()
    dead = false
    local coords = GetEntityCoords(cache.ped)
    ...
    exports.rr_api:Notify('EMS', 'Tu as ete reanime.', 'success')
end)

RegisterNetEvent('rr_medicextract:client:heal', function()
    SetEntityHealth(cache.ped, GetEntityMaxHealth(cache.ped))
    exports.rr_api:Notify('EMS', 'Tu as ete soigne.', 'success')
end)

RegisterNetEvent('rr_medicextract:client:extract', function()
    DoScreenFadeOut(500)
    Wait(600)
