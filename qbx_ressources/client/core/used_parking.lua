...
RegisterNetEvent('qbx_ressources:used:sync', function(data)
    syncListings(data)
end)

CreateThread(function()
    Wait(2500)
    local data = lib.callback.await('qbx_ressources:used:get', false)
...
local function openListingMenu(entry)
    local myId = lib.callback.await('qbx_ressources:getIdentifier', false)
    ...
    if isSeller then
        options[#options + 1] = {
            ...
            icon = 'xmark',
            onSelect = function()
                local ok, msg = lib.callback.await('qbx_ressources:used:remove', false, entry.id)
        ...
        options[#options + 1] = {
            ...
            icon = 'cart-shopping',
            onSelect = function()
                local ok, msg, plate, price = lib.callback.await('qbx_ressources:used:buy', false, entry.id)
                if ok then
    ...
    lib.registerContext({
        id = 'qbx_ressources_used_detail',
        title = Core.Locale('used_title'),
        options = options,
    })
    lib.showContext('qbx_ressources_used_detail')
end
...
local function listNearbyVehicle()
    ...
    local ok, msg, a, b = lib.callback.await('qbx_ressources:used:list', false, plate, input[1], props)
    if ok then
        SetEntityAsMissionEntity(veh, true, true)
        DeleteVehicle(veh)
