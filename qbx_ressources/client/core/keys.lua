...
local function toggleLock()
    ...
    local allowed = lib.callback.await('qbx_ressources:keys:canLock', false, plate)
    if not allowed then
    ...
    TriggerServerEvent('qbx_ressources:keys:setLock', netId, plate, locking)
end

RegisterNetEvent('qbx_ressources:keys:applyLock', function(netId, locking, plate, bySrc)
...
-- Item utilisable → verrouille le véhicule proche
RegisterNetEvent('qbx_ressources:keys:useItem', function()
    toggleLock()
end)
...
RegisterNetEvent('qbx_ressources:keys:registerHouse', function(houseId, coords, locked)
...
RegisterNetEvent('qbx_ressources:keys:setHouseLock', function(houseId, locked)
            ...
            if dist < (Config.Keys.houseDistance or 2.5) + 8.0 then
                ...
                if dist < (Config.Keys.houseDistance or 2.5) then
                    ...
                    if IsControlJustReleased(0, 38) then -- E
                        local ok = lib.callback.await('qbx_ressources:keys:canHouse', false, houseId)
                        if not ok then
                            Core.Notify(Core.Locale('key_house_no_key'), 'error')
                        else
                            TriggerServerEvent('qbx_ressources:keys:toggleHouse', houseId, not door.locked)
                        end
                    end
                end
            end
