...
local function openShop()
    local vehicles = lib.callback.await('qbx_ressources:keyshop:list', false) or {}
    ...
    if #vehicles == 0 then
        ...
        for _, v in ipairs(vehicles) do
            options[#options + 1] = {
                ...
                icon = 'key',
                onSelect = function()
                    local ok, msg, a, b = lib.callback.await('qbx_ressources:keyshop:buy', false, v.plate)
                    if ok then
    ...
    lib.registerContext({
        id = 'qbx_ressources_keyshop',
        title = Config.KeyShop.label or Core.Locale('keyshop_title'),
        options = options,
    })
    lib.showContext('qbx_ressources_keyshop')
end
