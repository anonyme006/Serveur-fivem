...
RegisterNetEvent('qbx_ressources:network:sync', function(dynamic, static)
...
local function setPhoneNetwork(hasSignal)
    ...
    LocalPlayer.state.qbx_ressources_signal = signalStrength
    LocalPlayer.state.qbx_ressources_has_network = hasSignal
end
...
--- Autres scripts : if not exports.qbx_ressources:CanUsePhoneFeature('call') then return end
...
AddEventHandler('qbx_ressources:network:tryCall', function(cb)
    local ok = signalStrength > 0
    if not ok then notifyNoSignal(Core.Locale('network_action_call')) end
    if cb then cb(ok) end
end)

AddEventHandler('qbx_ressources:network:tryMessage', function(cb)
    local ok = signalStrength > 0
    if not ok then notifyNoSignal(Core.Locale('network_action_sms')) end
    if cb then cb(ok) end
end)

AddEventHandler('qbx_ressources:network:trySocial', function(cb)
    local ok = signalStrength > 0
...
CreateThread(function()
    Wait(2000)
    local dyn, stat = lib.callback.await('qbx_ressources:network:getAntennas', false)
...
local function deployAntenna()
    ...
    local ok, msg, id = lib.callback.await('qbx_ressources:network:deploy', false, {
...
RegisterNetEvent('qbx_ressources:network:tryDeploy', function()
    deployAntenna()
end)
...
RegisterCommand('retirerantenne', function()
    ...
    local ok, msg = lib.callback.await('qbx_ressources:network:remove', false, nearestId)
