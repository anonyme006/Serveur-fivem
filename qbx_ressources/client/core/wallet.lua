...
local function openWallet()
    ...
    local wallet = lib.callback.await('qbx_ressources:wallet:get', false)
    if not wallet then return end

    local keys = {}
    if Config.Keys.enabled then
        keys = lib.callback.await('qbx_ressources:keys:list', false) or {}
    end
...
RegisterNUICallback('giveKey', function(data, cb)
    ...
    TriggerServerEvent('qbx_ressources:keys:give', input[1], keyType, keyRef, input[2] and true or false, input[3])
    cb(1)
end)

RegisterNUICallback('removeKey', function(data, cb)
    if data and data.id then
        TriggerServerEvent('qbx_ressources:keys:remove', data.id)
    end
    cb(1)
end)
...
RegisterCommand('trousseau', function()
    local keys = lib.callback.await('qbx_ressources:keys:list', false) or {}
    local options = {}
    ...
    if #keys == 0 then
        ...
        for _, key in ipairs(keys) do
            ...
            options[#options + 1] = {
                ...
                onSelect = function()
                    lib.registerContext({
                        id = 'qbx_ressources_key_actions',
                        title = title,
                        menu = 'qbx_ressources_keychain',
                        options = {
                            {
                                ...
                                onSelect = function()
                                    ...
                                    if input then
                                        TriggerServerEvent('qbx_ressources:keys:give', input[1], key.type, key.ref, input[2], input[3])
                                    end
                                end,
                            },
                            {
                                ...
                                onSelect = function()
                                    if key.id then
                                        TriggerServerEvent('qbx_ressources:keys:remove', key.id)
                                    end
                                end,
                            },
                        },
                    })
                    lib.showContext('qbx_ressources_key_actions')
                end,
            }
        end
    end
    ...
    lib.registerContext({
        id = 'qbx_ressources_keychain',
        title = Core.Locale('wallet_keys'),
        options = options,
    })
    lib.showContext('qbx_ressources_keychain')
end, false)
