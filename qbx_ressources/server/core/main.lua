...
--[[
    Bridge QBox — qbx_ressources
]]

local QBX = exports.qbx_core

-- SQL unifié chargé par server/bootstrap.lua
...
function Core.RemoveMoney(src, account, amount, reason)
    ...
    account = account == 'money' and 'cash' or (account or 'cash')
    return player.Functions.RemoveMoney(account, amount, reason or 'qbx_ressources')
end
...
function Core.AddMoney(src, account, accountAmount, reason)
    ...
    if not player then return false end
    account = account == 'money' and 'cash' or (account or 'cash')
    return player.Functions.AddMoney(account, accountAmount, reason or 'qbx_ressources')
end
...
function Core.RegisterUsableItem(item, cb)
    ...
    if not ok then
        print(('^3[qbx_ressources]^0 CreateUseableItem indisponible pour %s (utilise export ox_inventory)'):format(item))
    end
end
...
lib.callback.register('qbx_ressources:getIdentifier', function(source)
