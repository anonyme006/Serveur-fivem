...
local function broadcast(target)
    TriggerClientEvent('qbx_ressources:weather:sync', target or -1, syncPayload())
end
...
local function setWeather(weather, silent)
    ...
    if not silent and weatherCfg.notifyPlayers then
        TriggerClientEvent('qbx_ressources:notify', -1, Core.Locale('weather_changed', weather), 'inform')
    end
    return true
end
...
CreateThread(function()
    ...
    broadcast(-1)
    print(('^2[qbx_ressources]^0 météo sync : %s | %02d:%02d'):format(currentWeather, hour, minute))
end)
...
lib.callback.register('qbx_ressources:weather:get', function()
    return syncPayload()
end)
...
RegisterCommand('weather', function(src, args)
    if src > 0 and not isAdmin(src) then
        TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('weather_denied'), 'error')
        return
    end
    ...
    if not w or not VALID[w] then
        ...
        if src > 0 then
            TriggerClientEvent('qbx_ressources:notify', src, msg, 'inform')
        else
            print(msg)
        end
        return
    ...
    if src > 0 then
        TriggerClientEvent('qbx_ressources:notify', src, msg, 'success')
    else
        print(msg)
    end
    if Core.Log then
