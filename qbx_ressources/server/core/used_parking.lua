...
local function refreshCache()
    listings = {}
    slotTaken = {}
    local rows = MySQL.query.await('SELECT * FROM qbx_ressources_used_parking') or {}
    ...
    TriggerClientEvent('qbx_ressources:used:sync', -1, listings)
end
...
lib.callback.register('qbx_ressources:used:get', function()
    return listings
end)

lib.callback.register('qbx_ressources:used:list', function(source, plate, price, props)
    ...
    local existing = MySQL.single.await('SELECT id FROM qbx_ressources_used_parking WHERE plate = ? LIMIT 1', { plate })
    ...
    local insertId = MySQL.insert.await(
        'INSERT INTO qbx_ressources_used_parking (seller, plate, price, slot, vehicle, label) VALUES (?, ?, ?, ?, ?, ?)',
...
lib.callback.register('qbx_ressources:used:remove', function(source, listingId)
    local player = Core.GetPlayer(source)
    if not player then return false, 'cover_busy' end
    ...
    MySQL.update.await('DELETE FROM qbx_ressources_used_parking WHERE id = ?', { listingId })
...
lib.callback.register('qbx_ressources:used:buy', function(source, listingId)
    local player = Core.GetPlayer(source)
    if not player then return false, 'cover_busy' end
    ...
    if seller and seller.PlayerData and seller.PlayerData.source then
        ...
        TriggerClientEvent('qbx_ressources:notify', seller.PlayerData.source, Core.Locale('used_sold', sellerGain), 'success')
    else
    ...
    MySQL.update.await('DELETE FROM qbx_ressources_keys WHERE key_type = ? AND key_ref = ?', { 'vehicle', entry.plate })
    MySQL.insert.await(
