...
local function loadCovers()
    local rows = MySQL.query.await('SELECT * FROM qbx_ressources_covers') or {}
    covers = {}
    ...
    TriggerClientEvent('qbx_ressources:cover:sync', -1, covers)
end
...
lib.callback.register('qbx_ressources:cover:getAll', function()
    return covers
end)

lib.callback.register('qbx_ressources:cover:put', function(source, plate, coords, props)
    ...
    local allowed = false
    if Core.HasKey then
        allowed = Core.HasKey(Core.GetCitizenId(xPlayer), 'vehicle', plate, source)
    end
    if not allowed then
        ...
        local owned = MySQL.single.await(
            ...
            { Core.GetCitizenId(xPlayer), plate }
        )
        allowed = owned ~= nil
    end
    ...
    MySQL.insert.await(
        'INSERT INTO qbx_ressources_covers (plate, owner, coords, props) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE owner = VALUES(owner), coords = VALUES(coords), props = VALUES(props)',
        { plate, Core.GetCitizenId(xPlayer), json.encode(coords), json.encode(props or {}) }
    )

    covers[plate] = {
        owner = Core.GetCitizenId(xPlayer),
        coords = coords,
        props = props or {},
        plate = plate,
    }
...
lib.callback.register('qbx_ressources:cover:remove', function(source, plate)
    local xPlayer = Core.GetPlayer(source)
    ...
    local allowed = entry.owner == Core.GetCitizenId(xPlayer)
    ...
    MySQL.update.await('DELETE FROM qbx_ressources_covers WHERE plate = ?', { plate })
    covers[plate] = nil
...
AddEventHandler('playerJoining', function()
    ...
    SetTimeout(5000, function()
        TriggerClientEvent('qbx_ressources:cover:sync', src, covers)
    end)
end)
