...
local function hasKey(identifier, keyType, keyRef, src)
    ...
    local rows = MySQL.query.await(
        'SELECT * FROM qbx_ressources_keys WHERE holder = ? AND key_type = ? AND key_ref = ?',
    ...
    -- Propriétaire QBox = clé implicite (désactivable si requireItemToLock)
...
exports('GiveKey', function(holderIdentifier, keyType, keyRef, ownerIdentifier, label, temporary, minutes)
    ...
    return MySQL.insert.await(
        'INSERT INTO qbx_ressources_keys (owner, holder, key_type, key_ref, label, temporary, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        {
...
function Core.EnsureVehicleKey(src, plate, label, notifyKey)
    ...
    -- Sécurité : uniquement le propriétaire QBox
    local cols = Config.Persistence.columns
    local owned = MySQL.single.await(
        ('SELECT 1 FROM `%s` WHERE `%s` = ? AND REPLACE(UPPER(`%s`), " ", "") = ? LIMIT 1'):format(
            cols.table, cols.owner, cols.plate
    ...
    local exists = MySQL.single.await(
        'SELECT id FROM qbx_ressources_keys WHERE holder = ? AND key_type = ? AND key_ref = ? AND temporary = 0 LIMIT 1',
        { id, 'vehicle', plate }
    )
    ...
    if not exists then
        MySQL.insert.await(
            'INSERT INTO qbx_ressources_keys (owner, holder, key_type, key_ref, label, temporary) VALUES (?, ?, ?, ?, ?, 0)',
    ...
    if Config.Keys.notifyOnGive and notifyKey then
        TriggerClientEvent(
            'qbx_ressources:notify',
            src,
            Core.Locale(notifyKey, plate),
            created and 'success' or 'inform'
        )
    elseif Config.Keys.notifyOnGive and created then
        TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('key_received', plate), 'success')
    end
...
AddEventHandler('qbx_ressources:keys:giveVehicleKey', function(src, plate, label)
...
CreateThread(function()
    ...
    Core.RegisterUsableItem(item, function(playerId)
        TriggerClientEvent('qbx_ressources:keys:useItem', playerId)
    end)
end)

lib.callback.register('qbx_ressources:keys:canLock', function(source, plate)
    local id = Core.GetIdentifier(source)
    if not id then return false end
    return hasKey(id, 'vehicle', plate, source)
end)

lib.callback.register('qbx_ressources:keys:canHouse', function(source, houseId)
    local id = Core.GetIdentifier(source)
    if not id then return false end
    return hasKey(id, 'house', houseId, source)
end)

lib.callback.register('qbx_ressources:keys:list', function(source)
    local id = Core.GetIdentifier(source)
    if not id then return {} end

    local rows = MySQL.query.await(
        'SELECT id, key_type, key_ref, label, temporary, expires_at, owner FROM qbx_ressources_keys WHERE holder = ?',
        { id }
    ) or {}
...
RegisterNetEvent('qbx_ressources:keys:give', function(targetId, keyType, keyRef, temporary, minutes)
    local src = source
    ...
    local ok = hasKey(Core.GetCitizenId(xPlayer), keyType, keyRef, src)
    if not ok then
        TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('key_no_key'), 'error')
        return
    end
    ...
    if keyType == 'vehicle' then
        local owned = MySQL.single.await(
            ...
            { Core.GetCitizenId(xPlayer), keyRef }
        )
        isOwner = owned ~= nil
    else
        local row = MySQL.single.await(
            'SELECT 1 FROM qbx_ressources_keys WHERE owner = ? AND key_type = ? AND key_ref = ? LIMIT 1',
            { Core.GetCitizenId(xPlayer), keyType, keyRef }
        )
        isOwner = row ~= nil
    end
    ...
    MySQL.insert.await(
        'INSERT INTO qbx_ressources_keys (owner, holder, key_type, key_ref, label, temporary, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        {
            Core.GetCitizenId(xPlayer),
            Core.GetCitizenId(target),
            keyType,
            keyRef,
            keyRef,
            temporary and 1 or 0,
    ...
    TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('key_given', keyRef), 'success')
    TriggerClientEvent('qbx_ressources:notify', target.source, Core.Locale('key_received', keyRef), 'inform')
...
RegisterNetEvent('qbx_ressources:keys:remove', function(keyId)
    local src = source
    local xPlayer = Core.GetPlayer(src)
    ...
    local row = MySQL.single.await('SELECT * FROM qbx_ressources_keys WHERE id = ? LIMIT 1', { keyId })
    if not row then return end

    -- Propriétaire peut retirer n'importe quelle clé ; détenteur peut jeter la sienne
    if row.owner ~= Core.GetCitizenId(xPlayer) and row.holder ~= Core.GetCitizenId(xPlayer) then
        return
    end

    MySQL.update.await('DELETE FROM qbx_ressources_keys WHERE id = ?', { keyId })
    TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('key_removed'), 'success')
...
RegisterNetEvent('qbx_ressources:keys:setLock', function(netId, plate, locking)
    local src = source
    local id = Core.GetIdentifier(src)
    if not id then return end
    plate = Core.NormalizePlate(plate)
    ...
    TriggerClientEvent('qbx_ressources:keys:applyLock', -1, netId, locking and true or false, plate, src)
end)

RegisterNetEvent('qbx_ressources:keys:toggleHouse', function(houseId, locked)
    local src = source
    local id = Core.GetIdentifier(src)
    if not id then return end
    houseId = tostring(houseId or '')
    if not hasKey(id, 'house', houseId, src) then
        TriggerClientEvent('qbx_ressources:notify', src, Core.Locale('key_house_no_key'), 'error')
        return
    end
    TriggerClientEvent('qbx_ressources:keys:setHouseLock', -1, houseId, locked and true or false)
    TriggerClientEvent(
        'qbx_ressources:notify',
        src,
...
exports('RegisterHouseDoor', function(houseId, coords, locked)
    TriggerClientEvent('qbx_ressources:keys:registerHouse', -1, houseId, coords, locked ~= false)
end)
