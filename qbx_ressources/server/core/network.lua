...
local function syncAll(target)
    TriggerClientEvent('qbx_ressources:network:sync', target or -1, antennas, Config.Network.staticAntennas or {})
end

local function loadAntennas()
    antennas = {}
    local rows = MySQL.query.await('SELECT * FROM qbx_ressources_antennas') or {}
    ...
    syncAll(-1)
    print(('^2[qbx_ressources]^0 %d antenne(s) réseau chargée(s)'):format(#rows))
end
...
lib.callback.register('qbx_ressources:network:getSignal', function(source)
    local strength, antennaId = Core.GetSignalStrength(source)
    return { strength = strength, antennaId = antennaId, hasSignal = strength > 0 }
end)

lib.callback.register('qbx_ressources:network:getAntennas', function()
    return antennas, Config.Network.staticAntennas or {}
end)

lib.callback.register('qbx_ressources:network:deploy', function(source, coords, heading)
    ...
    local insertId = MySQL.insert.await(
        'INSERT INTO qbx_ressources_antennas (owner, label, coords, heading, range_m) VALUES (?, ?, ?, ?, ?)',
        {
    ...
    MySQL.update.await('UPDATE qbx_ressources_antennas SET label = ? WHERE id = ?', {
...
lib.callback.register('qbx_ressources:network:remove', function(source, antennaId)
    local player = Core.GetPlayer(source)
    if not player then return false, 'cover_busy' end
    ...
    MySQL.update.await('DELETE FROM qbx_ressources_antennas WHERE id = ?', { antennaId })
...
lib.callback.register('qbx_ressources:network:canUsePhone', function(source, action)
...
CreateThread(function()
    ...
    Core.RegisterUsableItem(item, function(playerId)
        TriggerClientEvent('qbx_ressources:network:tryDeploy', playerId)
    end)
end)
