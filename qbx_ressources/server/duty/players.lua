...
function Duty.BroadcastBlipRefresh(affectedJobs)
    for viewerSrc, viewer in pairs(DutyPlayers) do
        if GetPlayerName(viewerSrc) then
            ...
            if shouldSync then
                TriggerClientEvent('qbx_ressources:duty:client:updateBlips', viewerSrc, Duty.GetVisibleForViewer(viewerSrc))
            end
        end
    end
end
    ...
    for viewerSrc in pairs(DutyPlayers) do
        if GetPlayerName(viewerSrc) then
            ...
            if next(coordsPayload) then
                TriggerClientEvent('qbx_ressources:duty:client:updateCoords', viewerSrc, coordsPayload)
            end
        end
    end
end
...
function Duty.SyncViewer(src)
    ...
    TriggerClientEvent('qbx_ressources:duty:client:updateBlips', src, Duty.GetVisibleForViewer(src))
end
