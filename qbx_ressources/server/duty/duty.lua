...
function Duty.SetDutyState(src, desired, opts)
    ...
    if desired then
        ...
        TriggerClientEvent('qbx_ressources:duty:client:onDuty', src, {
            job = jobName,
        ...
        TriggerClientEvent('qbx_ressources:duty:client:offDuty', src, {
            job = jobName,
            label = Duty.GetJobLabel(jobName),
            grade = grade,
        })
...
RegisterNetEvent('qbx_ressources:duty:server:requestBlipSync', function()
    Duty.SyncViewer(source)
end)

RegisterNetEvent('qbx_ressources:duty:server:setDuty', function(desired, skipPointCheck)
    local src = source
    if type(desired) ~= 'boolean' then return end
    local ok, reason = Duty.SetDutyState(src, desired, { skipPointCheck = skipPointCheck == true })
    if not ok then
        TriggerClientEvent('qbx_ressources:duty:client:dutyDenied', src, reason)
    end
end)

lib.callback.register('qbx_ressources:duty:server:getDuty', function(source)
    return Duty.GetDuty(source)
end)

lib.callback.register('qbx_ressources:duty:server:getOnDutyCount', function(_, jobName)
    if type(jobName) ~= 'string' then return 0 end
    return Duty.GetOnDutyCount(jobName)
end)

lib.callback.register('qbx_ressources:duty:server:getEmployeesOnDuty', function(_, jobName)
    if type(jobName) ~= 'string' then return {} end
    return Duty.GetEmployeesOnDuty(jobName)
end)

lib.callback.register('qbx_ressources:duty:server:toggleDuty', function(source)
