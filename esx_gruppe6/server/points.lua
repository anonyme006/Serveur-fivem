...
function Gruppe6Points.Load()
    local rows = MySQL.query.await('SELECT * FROM esx_gruppe6_points ORDER BY id ASC') or {}
    local list = {}
    for i = 1, #rows do
...
function Gruppe6Points.SeedDefaults()
    local count = MySQL.scalar.await('SELECT COUNT(*) FROM esx_gruppe6_points') or 0
    if count > 0 then return end

    for _, point in ipairs(Config.DefaultPoints) do
        MySQL.insert.await([[
            INSERT INTO esx_gruppe6_points (point_type, label, x, y, z, enabled)
            VALUES (?, ?, ?, ?, ?, 1)
...
function Gruppe6Points.GetEnabled()
    local rows = MySQL.query.await('SELECT * FROM esx_gruppe6_points WHERE enabled = 1 ORDER BY id ASC') or {}
    local list = {}
    for i = 1, #rows do
        list[#list + 1] = rowToPoint(rows[i])
    end
...
function Gruppe6Points.GetById(id)
    local row = MySQL.single.await('SELECT * FROM esx_gruppe6_points WHERE id = ?', { id })
...
function Gruppe6Points.Delete(id)
    return MySQL.update.await('DELETE FROM esx_gruppe6_points WHERE id = ?', { id }) > 0
end

function Gruppe6Points.Toggle(id, enabled)
    return MySQL.update.await('UPDATE esx_gruppe6_points SET enabled = ? WHERE id = ?', { enabled and 1 or 0, id }) > 0
end
