...
local function syncAll(target)
    TriggerClientEvent('esx_core:network:sync', target or -1, antennas, Config.Network.staticAntennas or {})
end

local function loadAntennas()
    antennas = {}
    local rows = MySQL.query.await('SELECT * FROM esx_core_antennas') or {}
    ...
    syncAll(-1)
    print(('^2[esx_core]^0 %d antenne(s) réseau chargée(s)'):format(#rows))
end
...
local function removeItem(src)
    ...
    local inv = xPlayer.getInventoryItem and xPlayer.getInventoryItem(item)
    if not inv or (inv.count or 0) < 1 then return false end
...
local function hasItem(src)
    ...
    if not xPlayer then return false end
    local inv = xPlayer.getInventoryItem and xPlayer.getInventoryItem(item)
    return inv and (inv.count or 0) > 0
end
...
lib.callback.register('esx_core:network:getSignal', function(source)
    local strength, antennaId = Core.GetSignalStrength(source)
    return { strength = strength, antennaId = antennaId, hasSignal = strength > 0 }
end)

lib.callback.register('esx_core:network:getAntennas', function()
    return antennas, Config.Network.staticAntennas or {}
end)

lib.callback.register('esx_core:network:deploy', function(source, coords, heading)
    ...
    if max > 0 and countOwned(xPlayer.identifier) >= max then
    ...
    local insertId = MySQL.insert.await(
        'INSERT INTO esx_core_antennas (owner, label, coords, heading, range_m) VALUES (?, ?, ?, ?, ?)',
        {
            xPlayer.identifier,
            ('Antenne %s'):format(GetPlayerName(source) or insertId or ''),
    ...
    -- Fix label after insert
    MySQL.update.await('UPDATE esx_core_antennas SET label = ? WHERE id = ?', {
...
lib.callback.register('esx_core:network:remove', function(source, antennaId)
    local xPlayer = Core.GetPlayer(source)
    if not xPlayer then return false, 'cover_busy' end
    ...
    if not allowed and Config.Network.removeJobs and xPlayer.job then
        local minGrade = Config.Network.removeJobs[xPlayer.job.name]
        if minGrade ~= nil and (xPlayer.job.grade or 0) >= minGrade then
            allowed = true
        end
    end
    if not allowed then return false, 'network_not_yours' end

    MySQL.update.await('DELETE FROM esx_core_antennas WHERE id = ?', { antennaId })
...
--- Autoriser une action téléphone (appel / sms / social)
lib.callback.register('esx_core:network:canUsePhone', function(source, action)
...
CreateThread(function()
    ...
    if ESX and ESX.RegisterUsableItem then
        ESX.RegisterUsableItem(item, function(playerId)
            TriggerClientEvent('esx_core:network:tryDeploy', playerId)
        end)
    end
end)
