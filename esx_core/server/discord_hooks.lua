--[[
    Hooks logs Discord — événements serveur + bridge esx_core
]]
...
AddEventHandler('onResourceStop', function(res)
    if res == GetCurrentResourceName() then
        log('system', '🔴 esx_core stoppé', nil, { color = 'error' })
        return
    end
...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
    ...
    local job = xPlayer and xPlayer.job and xPlayer.job.name or '?'
    log('connect', '👤 ESX chargé', ('Job : `%s`'):format(job), {
        color = 'success',
        src = src,
    })
end)
...
--- Events internes esx_core (émis depuis les modules)
AddEventHandler('esx_core:log', function(category, title, description, opts)
