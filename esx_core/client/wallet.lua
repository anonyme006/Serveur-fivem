...
local function openWallet()
    ...
    local wallet = lib.callback.await('esx_core:wallet:get', false)
    if not wallet then return end

    local keys = {}
    if Config.Keys.enabled then
        keys = lib.callback.await('esx_core:keys:list', false) or {}
    end
    ...
    SendNUIMessage({
        ...
        name = wallet.name,
        accounts = {
            { id = 'money', label = Core.Locale('wallet_cash'), amount = wallet.accounts.money or 0 },
...
RegisterNUICallback('giveKey', function(data, cb)
    ...
    TriggerServerEvent('esx_core:keys:give', input[1], keyType, keyRef, input[2] and true or false, input[3])
    cb(1)
end)

RegisterNUICallback('removeKey', function(data, cb)
    if data and data.id then
        TriggerServerEvent('esx_core:keys:remove', data.id)
    end
    cb(1)
end)
...
RegisterCommand('trousseau', function()
    local keys = lib.callback.await('esx_core:keys:list', false) or {}
    local options = {}
    ...
    if #keys == 0 then
        ...
        for _, key in ipairs(keys) do
            ...
            options[#options + 1] = {
                ...
                onSelect = function()
                    lib.registerContext({
                        id = 'esx_core_key_actions',
                        title = title,
                        menu = 'esx_core_keychain',
                        options = {
                            {
                                ...
                                onSelect = function()
                                    ...
                                    if input then
                                        TriggerServerEvent('esx_core:keys:give', input[1], key.type, key.ref, input[2], input[3])
                                    end
                                end,
                            },
                            {
                                ...
                                onSelect = function()
                                    if key.id then
                                        TriggerServerEvent('esx_core:keys:remove', key.id)
                                    end
                                end,
                            },
                        },
                    })
                    lib.showContext('esx_core_key_actions')
                end,
            }
        end
    end
    ...
    lib.registerContext({
        id = 'esx_core_keychain',
        title = Core.Locale('wallet_keys'),
        options = options,
    })
    lib.showContext('esx_core_keychain')
end, false)
