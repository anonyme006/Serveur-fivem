...
RegisterNetEvent('esx_core:used:sync', function(data)
    syncListings(data)
end)

CreateThread(function()
    Wait(2500)
    local data = lib.callback.await('esx_core:used:get', false)
...
local function openListingMenu(entry)
    local myId = lib.callback.await('esx_core:getIdentifier', false)
    ...
    if isSeller then
        options[#options + 1] = {
            ...
            icon = 'xmark',
            onSelect = function()
                local ok, msg = lib.callback.await('esx_core:used:remove', false, entry.id)
        ...
        options[#options + 1] = {
            ...
            icon = 'cart-shopping',
            onSelect = function()
                local ok, msg, plate, price = lib.callback.await('esx_core:used:buy', false, entry.id)
                if ok then
    ...
    lib.registerContext({
        id = 'esx_core_used_detail',
        title = Core.Locale('used_title'),
        options = options,
    })
    lib.showContext('esx_core_used_detail')
end
...
local function listNearbyVehicle()
    ...
    local ok, msg, a, b = lib.callback.await('esx_core:used:list', false, plate, input[1], props)
    if ok then
        SetEntityAsMissionEntity(veh, true, true)
        DeleteVehicle(veh)
