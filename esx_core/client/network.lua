...
RegisterNetEvent('esx_core:network:sync', function(dynamic, static)
...
local function setPhoneNetwork(hasSignal)
    ...
    LocalPlayer.state.esx_core_signal = signalStrength
    LocalPlayer.state.esx_core_has_network = hasSignal
end
...
--- Autres scripts : if not exports.esx_core:CanUsePhoneFeature('call') then return end
...
AddEventHandler('esx_core:network:tryCall', function(cb)
    local ok = signalStrength > 0
    if not ok then notifyNoSignal(Core.Locale('network_action_call')) end
    if cb then cb(ok) end
end)

AddEventHandler('esx_core:network:tryMessage', function(cb)
    local ok = signalStrength > 0
    if not ok then notifyNoSignal(Core.Locale('network_action_sms')) end
    if cb then cb(ok) end
end)

AddEventHandler('esx_core:network:trySocial', function(cb)
    local ok = signalStrength > 0
...
CreateThread(function()
    Wait(2000)
    local dyn, stat = lib.callback.await('esx_core:network:getAntennas', false)
...
local function deployAntenna()
    ...
    local ok, msg, id = lib.callback.await('esx_core:network:deploy', false, {
...
RegisterNetEvent('esx_core:network:tryDeploy', function()
    deployAntenna()
end)
...
RegisterCommand('retirerantenne', function()
    ...
    local ok, msg = lib.callback.await('esx_core:network:remove', false, nearestId)
