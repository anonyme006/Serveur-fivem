...
Utilise **`esx_progressbar`** (capsule orange) pour :

- Sortie de véhicule
- Rangement
- Mise en fourrière / sortie fourrière

Fallback automatique sur `lib.progressCircle` si `esx_progressbar` est absent.
...
> Le dossier ressource est `pa_garage` (exports internes `ox_garage`).
