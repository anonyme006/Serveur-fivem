...
local function refreshLocalState()
    local data = lib.callback.await('qbx-duty:server:getDuty', false)
    if data then
...
RegisterNetEvent('qbx-duty:client:dutyDenied', function(reason)
    local messages = {
...
RegisterNetEvent('qbx-duty:client:onDuty', function(payload)
    myDuty = true
...
RegisterNetEvent('qbx-duty:client:offDuty', function(payload)
    myDuty = false
    myJob = payload and payload.job or myJob
                    ...
                    options = {
                        {
                            ...
                            onSelect = function()
                                TriggerServerEvent('qbx-duty:server:setDuty', true)
                            end,
                        },
                        {
                            ...
                            onSelect = function()
                                TriggerServerEvent('qbx-duty:server:setDuty', false)
                            end,
                        },
                    },
                })
...
if Config.Command and Config.Command.enabled and Config.Command.name then
    RegisterCommand(Config.Command.name, function()
        local ok, reason, data = lib.callback.await('qbx-duty:server:toggleDuty', false)
...
exports('SetDuty', function(state)
    TriggerServerEvent('qbx-duty:server:setDuty', state == true)
end)

exports('GetOnDutyCount', function(jobName)
    return lib.callback.await('qbx-duty:server:getOnDutyCount', false, jobName)
end)

exports('GetEmployeesOnDuty', function(jobName)
    return lib.callback.await('qbx-duty:server:getEmployeesOnDuty', false, jobName)
end)

exports('IsJobOnDuty', function(jobName)
    return lib.callback.await('qbx-duty:server:getOnDutyCount', false, jobName) > 0
end)
