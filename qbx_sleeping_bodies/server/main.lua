...
function RemoveSleepingBody(citizenid)
    ...
    deleteSql(citizenid)
    TriggerClientEvent('qbx_sleeping_bodies:client:remove', -1, citizenid)
...
local function createBodyFromPlayer(source, reason)
    ...
    if cache and cache.coords then
        if not coords then
            ...
            if delta > (Config.MaxDropDistanceDelta or 80.0) then
    ...
    local body = {
        ...
        x = coords.x + 0.0,
        y = coords.y + 0.0,
        z = coords.z + (Config.ZOffset or 0.0),
        heading = (heading or 0.0) + (Config.RotationOffset or 0.0),
        bucket = bucket,
    }
    ...
    broadcast('qbx_sleeping_bodies:client:add', body, bucket)
    -- Aussi envoyer à tous : le client filtre par bucket local
    TriggerClientEvent('qbx_sleeping_bodies:client:add', -1, body)
...
RegisterNetEvent('qbx_sleeping_bodies:server:cache', function(payload)
    local src = source
...
local function onPlayerLoaded(player)
    ...
    if Config.DeleteOnReconnect ~= false and Bodies[citizenid] then
...
lib.callback.register('qbx_sleeping_bodies:getBodies', function(source)
    local bucket = getBucket(source)
    local list = {}
...
lib.callback.register('qbx_sleeping_bodies:getBucket', function(source)
    return getBucket(source)
end)

lib.callback.register('qbx_sleeping_bodies:admin:list', function(source)
...
RegisterNetEvent('qbx_sleeping_bodies:server:adminRemove', function(citizenid)
    local src = source
...
RegisterNetEvent('qbx_sleeping_bodies:server:adminRemoveAll', function()
    local src = source
    if not isAdmin(src) then return end
    local ids = {}
...
RegisterNetEvent('qbx_sleeping_bodies:server:adminTeleportBody', function(citizenid)
    local src = source
    if not isAdmin(src) then return end
    citizenid = tostring(citizenid or '')
    ...
    body.x, body.y, body.z = coords.x, coords.y, coords.z + (Config.ZOffset or 0.0)
    body.heading = heading + (Config.RotationOffset or 0.0)
    body.bucket = bucket
    Bodies[citizenid] = body
    upsertSql(body)
...
local function loadBodiesFromDb()
    if Config.LoadBodiesOnResourceStart == false then return end
    ...
    TriggerClientEvent('qbx_sleeping_bodies:client:fullSync', -1, Bodies)
end

MySQL.ready(function()
    local sql = LoadResourceFile(GetCurrentResourceName(), 'sql/sleeping_bodies.sql')
    if sql then
...
AddEventHandler('onResourceStop', function(res)
    if res ~= GetCurrentResourceName() then return end
    TriggerClientEvent('qbx_sleeping_bodies:client:clear', -1)
end)
