...
local function detectAppearanceSystem()
    local configured = Config.AppearanceSystem or 'auto'
...
function PlaySleepingAnimation(ped)
    ...
    if Config.UseScenario and Config.Scenario then
        TaskStartScenarioInPlace(ped, Config.Scenario, 0, true)
        return
    end

    local primary = Config.SleepAnimation or {}
    local fallback = Config.SleepAnimationFallback or {}
...
local function addTarget(citizenid, ped, body)
    if not Config.UseOxTarget or not isResourceStarted('ox_target') then return end
    ...
    local options = {
        ...
        },
        {
            ...
            onSelect = function()
                TriggerServerEvent('qbx_sleeping_bodies:server:adminRemove', citizenid)
            end,
        },
        {
            ...
            groups = Config.AdminGroups,
            onSelect = function()
                TriggerServerEvent('qbx_sleeping_bodies:server:adminTeleportBody', citizenid)
            end,
        },
    }
...
local function createBodyPed(body)
    ...
    local ped = CreatePed(
        ...
        body.x + 0.0,
        body.y + 0.0,
        body.z + (Config.ZOffset or 0.0),
        (body.heading or 0.0) + (Config.RotationOffset or 0.0),
    ...
    SetEntityCoordsNoOffset(ped, body.x, body.y, body.z + (Config.ZOffset or 0.0), false, false, false)
    SetEntityHeading(ped, (body.heading or 0.0) + (Config.RotationOffset or 0.0))

    local found, groundZ = GetGroundZFor_3dCoord(body.x + 0.0, body.y + 0.0, body.z + 2.0, false)
    if found then
        SetEntityCoordsNoOffset(ped, body.x, body.y, groundZ + (Config.ZOffset or 0.0), false, false, false)
    end
...
local function refreshBucketVisibility()
    myBucket = lib.callback.await('qbx_sleeping_bodies:getBucket', false) or 0
    local list = lib.callback.await('qbx_sleeping_bodies:getBodies', false) or {}
    local keep = {}
    for i = 1, #list do
        local body = list[i]
...
RegisterNetEvent('qbx_sleeping_bodies:client:add', function(body)
    body = SleepBodies.NormalizeBody(body)
    if not body then return end
    myBucket = lib.callback.await('qbx_sleeping_bodies:getBucket', false) or myBucket
...
RegisterNetEvent('qbx_sleeping_bodies:client:remove', function(citizenid)
    citizenid = tostring(citizenid or '')
    deleteBodyPed(citizenid)
end)

RegisterNetEvent('qbx_sleeping_bodies:client:clear', function()
    clearAllPeds()
end)

RegisterNetEvent('qbx_sleeping_bodies:client:fullSync', function(map)
    clearAllPeds()
...
CreateThread(function()
    while true do
        Wait(Config.ClientCacheInterval or 15000)
        ...
        if ped ~= 0 and not IsEntityDead(ped) then
            ...
            TriggerServerEvent('qbx_sleeping_bodies:server:cache', {
...
CreateThread(function()
    ...
    while true do
        ...
        local nearAny = false
        local nameDist = Config.NameDistance or 10.0
        ...
        for citizenid, ped in pairs(LocalPeds) do
            if not DoesEntityExist(ped) then
                ...
                if dist < 40.0 then
                    ...
                    if not IsEntityPlayingAnim(ped, (Config.SleepAnimation and Config.SleepAnimation.dict) or '', (Config.SleepAnimation and Config.SleepAnimation.anim) or '', 3)
                        and not IsEntityPlayingAnim(ped, (Config.SleepAnimationFallback and Config.SleepAnimationFallback.dict) or '', (Config.SleepAnimationFallback and Config.SleepAnimationFallback.anim) or '', 3)
                    then
                        PlaySleepingAnimation(ped)
                    end
                end

                if Config.ShowName and dist <= nameDist then
                    sleep = 0
                    ...
                    if onScreen then
                        local c = Config.NameColor or { r = 180, g = 210, b = 255, a = 220 }
...
CreateThread(function()
    while true do
        Wait(5000)
        local bucket = lib.callback.await('qbx_sleeping_bodies:getBucket', false)
...
--- Menu admin ox_lib
RegisterNetEvent('qbx_sleeping_bodies:client:adminMenu', function()
    local list = lib.callback.await('qbx_sleeping_bodies:admin:list', false)
    if list == nil then
        ...
        },
        {
            ...
            onSelect = function()
                ...
                if confirm == 'confirm' then
                    TriggerServerEvent('qbx_sleeping_bodies:server:adminRemoveAll')
                end
            end,
        },
        {
                ...
                lib.registerContext({
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            icon = 'location-arrow',
                            onSelect = function()
                                TriggerServerEvent('qbx_sleeping_bodies:server:adminTeleportBody', found.citizenid)
                            end,
                        },
                        {
                            title = 'Supprimer',
                            icon = 'trash',
                            onSelect = function()
                                TriggerServerEvent('qbx_sleeping_bodies:server:adminRemove', found.citizenid)
                            end,
                        },
                    },
                })
                ...
                lib.registerContext({
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            icon = 'location-arrow',
                            onSelect = function()
                                TriggerServerEvent('qbx_sleeping_bodies:server:adminTeleportBody', body.citizenid)
                            end,
                        },
                        {
                            title = 'Supprimer',
                            icon = 'trash',
                            onSelect = function()
                                TriggerServerEvent('qbx_sleeping_bodies:server:adminRemove', body.citizenid)
                            end,
                        },
                    },
                })
